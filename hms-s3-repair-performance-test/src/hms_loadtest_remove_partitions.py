...
def loadtest_remove(num_tables=1000, year=2009, month=12):
    ...
    for table_num in range(0, num_tables):
        output = hms_loadtest_base.remove_partition(year, month, table_num)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        try:
            ...
            print("Usage: python hms_loadtest_remove_partitions.py [num_tables] [year] [month]")
            print("Example: python hms_loadtest_remove_partitions.py 500 2010 6")
            sys.exit(1)
    
    print("Starting partition removal test with:")
    print(f"  Number of tables: {num_tables}")
    print(f"  Target partition: {year}-{month:02d}")
    
    loadtest_remove(num_tables, year, month)
